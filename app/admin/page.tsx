...
export default async function AdminDashboard() {
  ...
  if (isUser) {
    return (
      <div className="h-[60vh] flex flex-col items-center justify-center text-center px-4">
        <div className="h-20 w-20 rounded-3xl bg-brand-forest/10 text-brand-forest flex items-center justify-center mb-6 shadow-sm border border-brand-forest/10">
           <ShieldAlert size={40} />
        </div>
        <h2 className="text-2xl sm:text-3xl font-black text-slate-900 mb-2">Account Pending Upgrade</h2>
        <div className="max-w-md space-y-4">
          <p className="text-slate-500 font-medium">
            Welcome to the Rooted Rising Administrative Portal. Your account has been successfully registered but hasn't been assigned an operational role yet.
          </p>
          <div className="bg-white border border-slate-100 rounded-2xl p-6 text-left shadow-sm">
            <h4 className="text-sm font-black text-slate-900 uppercase tracking-widest mb-3">Next Steps:</h4>
            <ul className="text-xs text-slate-500 font-bold space-y-2 list-disc pl-4">
              <li>Contact your supervisor or the system administrator to request access.</li>
              <li>Provide them with your registered email address (<span className="text-brand-forest">{dbUser.email}</span>).</li>
              <li>You will receive an email confirmation once your access levels are updated.</li>
              <li>In the meantime, you can update your credentials in the <Link href="/admin/settings/password" className="text-brand-forest underline hover:text-brand-dark transition-colors">Security Settings</Link>.</li>
            </ul>
          </div>
        </div>
      </div>
  ...
  const stats = [
    ...(isAdmin ? [{ name: "Total Users", value: userCount, icon: <Users />, color: "bg-blue-50 text-blue-600" }] : []),
    { name: "Blog Posts", value: postCount, icon: <FileText />, color: "bg-emerald-50 text-emerald-600" },
    ...(isAdmin ? [{ name: "Newsletter", value: subscriberCount, icon: <Mail />, color: "bg-purple-50 text-purple-600" }] : []),
    ...(isAdmin ? [{ name: "Comments", value: commentCount, icon: <MessageSquare />, color: "bg-amber-50 text-amber-600" }] : []),
  ];

  return (
    <div className="space-y-6 sm:space-y-10">
      <div className={`grid grid-cols-1 sm:grid-cols-2 ${isAdmin ? 'lg:grid-cols-4' : 'lg:grid-cols-1'} gap-4 sm:gap-8`}>
        {stats.map((stat) => (
      ...
      {isEditor && (
        <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100 text-center">
          <p className="text-slate-500 font-medium italic">Welcome back! You have Editor access. Use the sidebar to manage content.</p>
        </div>
      )}
    </div>
  );
